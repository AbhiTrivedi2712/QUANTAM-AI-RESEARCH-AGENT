...
export default function RecommendationCard({ finalDecision, symbol, technical, fundamental, sentiment }) {
  ...
  const gaugeData = [
    ...
    { value: 100 - (confidence || 50), fill: "#0b1020" }
  ];

  // Risk meter position helper
  const riskPercent = risk === "Low" ? 20 : risk === "Medium" ? 50 : 80;
  const riskColor = risk === "Low" ? "bg-emerald-400 shadow-emerald-400/30" : risk === "High" ? "bg-red-400 shadow-red-400/30" : "bg-amber-400 shadow-amber-400/30";

  return (
    <div
      className={`rounded-2xl border ${theme.border} bg-gradient-to-br ${theme.bg} p-6 mb-6 fade-in-up relative overflow-hidden`}
      style={{ boxShadow: `0 4px 20px -2px rgba(0, 0, 0, 0.4), 0 0 1px 1px rgba(255, 255, 255, 0.03)`, animationDelay: "0.2s" }}
    >
      <div className="absolute -right-24 -top-24 w-64 h-64 rounded-full bg-blue-500/5 blur-3xl pointer-events-none" />

      {/* ── Executive Brief banner ── */}
      <div className="bg-[#0b1020]/80 border border-white/5 rounded-xl p-5 mb-6 shadow-inner relative overflow-hidden">
        <div className="absolute -left-20 -bottom-20 w-44 h-44 rounded-full bg-blue-500/5 blur-3xl pointer-events-none" />
        <div className="flex items-center gap-2 mb-2">
          <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-ping" />
          <span className="text-[10px] font-bold uppercase tracking-wider text-blue-400 font-mono">Consolidated Executive Brief</span>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 flex flex-col items-center justify-center text-center p-5 bg-[#060816]/80 rounded-xl border border-white/5 relative">
          ...
          <div className="w-full mt-4 space-y-2">
            ...
            <div className="h-1 rounded-full bg-[#060816] relative">
              <div 
        ...
        <div className="lg:col-span-8 flex flex-col justify-between">
          ...
          {technical && fundamental && sentiment && (
            <div className="bg-[#060816]/80 rounded-xl border border-white/5 p-4 mb-4">
              ...
              <div className="grid grid-cols-3 gap-3 text-center text-xs font-mono">
                <div className="bg-[#0b1020]/60 p-3 rounded-lg border border-white/5 flex flex-col justify-between">
            ...
            <div>
              ...
              <ul className="space-y-1.5">
                {key_drivers && key_drivers.slice(0, 2).map((drv, idx) => (
                  <li key={idx} className="flex gap-2 items-start bg-[#060816]/50 p-2.5 rounded-lg border border-white/5 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 border-t border-white/5 pt-5 text-xs">
        <div>
          ...
          <ul className="space-y-1.5">
            {potential_risks && potential_risks.slice(0, 2).map((rsk, i) => (
              <li key={i} className="flex gap-2 items-start bg-[#060816]/40 p-2.5 rounded-lg border border-white/5 font-mono text-[10px]">
